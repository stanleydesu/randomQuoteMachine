(function(){
  ...
  const quote = document.getElementById('quote'),
        ...
        nextQuote = document.getElementById('next'),
  ...
  // loads new quote and refreshes background color
  function updatePage() {
    changeBackgroundColor();
    loadQuote();
  }

  function pickRandom(arr, old) {
    let index = Math.floor(Math.random() * arr.length);
    return arr[index] === old ? arr[(index + 1) % arr.length] : arr[index];
  }

  // function to refresh quote-box
  function loadQuote() {
    let currentQuoteId = quote.getAttribute('data-id');
    let quoteObj;
    let quoteIndex = Math.floor(Math.random() * quoteArray.length);
    if (quoteArray[quoteIndex].id == currentQuoteId) {
      ++quoteIndex;
      quoteIndex % quoteArray.length;
    }
    quoteObj = quoteArray[quoteIndex];
    // load in quote text and author
    quote.textContent = quoteObj.quote;
    author.textContent = '- ' + quoteObj.author;
    // update data-id of quote div and href of tweet audiencehor
    quote.setAttribute('data-id', quoteObj.id);
    tweet.setAttribute('href', "https://twitter.com/intent/tweet?text=" + encodeURIComponent(quoteObj.quote) + "&output=embed");
  }


  function changeBackgroundColor() {
    let colors = ['#95a5a6','#34495e','#e74c3c','#e67e22','#f1c40f','#2ecc71','#1abc9c','#3498db','#9b59b6'];
    // current color
    let currentColor = document.documentElement.style.getPropertyValue('--color');
    let newColor = pickRandom(colors, currentColor);
    // set the css root variable of --color to the new color
    document.documentElement.style.setProperty('--color', newColor);
  }

  // event listeners
  nextQuote.addEventListener('click', updatePage, false);
  body.addEventListener('touchstart', function(){});

  // on startup, get new quote and background color
  updatePage();
})();
